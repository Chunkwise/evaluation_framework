...
class SyntheticEvaluation(BaseEvaluation):
    def __init__(
        ...
        with resources.as_file(
            ...
            with open(
                os.path.join(prompt_path, "question_maker_approx_system.txt"), "r"
            ) as f:
            ...
            with open(
                os.path.join(prompt_path, "question_maker_approx_user.txt"), "r"
            ) as f:
    ...
    def _extract_question_and_approx_references(
        ...
        for reference in text_references:
            ...
            if "start_chunk" not in reference_keys or "end_chunk" not in reference_keys:
                raise ValueError(
                    "Each reference must contain 'start_chunk' and 'end_chunk' keys."
                )
        ...
        while i < n:
            while True:
                try:
                    ...
                    # Use override if available, otherwise use original corpus_id
                    stored_corpus_id = self.corpus_id_override.get(corpus_id, corpus_id)

                    questions_list = self.synth_questions_df[
                        self.synth_questions_df["corpus_id"] == stored_corpus_id
                    ]["question"].tolist()
                    if approx:
                        question, references = (
                            self._extract_question_and_approx_references(
    ...
    def _corpus_filter_poor_highlights(self, corpus_id, synth_questions_df, threshold):
        ...
        corpus_questions_df["references"] = corpus_questions_df["references"].apply(
            json.dumps
        )
    ...
    def filter_poor_excerpts(self, threshold=0.36, corpora_subset=[]):
        if os.path.exists(self.questions_csv_path):
            ...
            if len(synth_questions_df) > 0:
                synth_questions_df["references"] = synth_questions_df[
                    "references"
                ].apply(json.loads)
    ...
    def _corpus_filter_duplicates(self, corpus_id, synth_questions_df, threshold):
        ...
        corpus_questions_df.drop_duplicates(
            subset="question", keep="first", inplace=True
        )
        ...
        most_similars = (dot_product_matrix - np.eye(dot_product_matrix.shape[0])).max(
            axis=1
        )
                ...
                if remaining[i] == 1:  # Only check for vectors that are still remaining
                    for j in range(i + 1, n):
                        if remaining[j] == 1 and sim_matrix[i, j] > threshold:
                            remaining[j] = (
                                0  # Remove vector j because it's too similar to vector i
                            )
